...
import DateTimeSelector from '@/src/components/time/date-time-select';
import { Textarea } from "@/components/ui/textarea";
import DatesSelect from '@/src/components/time/dates-select';
import { Button } from '@/src/components/ui/button';
import LeaveHistory from "@/src/components/leave/leaveHistory";
...
import { HolidaysType } from "@/src/types/leaves";
...
const calculateMinForEndDate=(startDate:Date):Date=>{
  ...
  const data = await fetchResourceById(session?.user.id);
        if (data) {
          const { vocationLeaves, vocationLeavesAvailable, vocationLeavesConsumed, sickLeaves, sickLeavesConsumed, sickLeavesAvailable,vactionLeaveNoticePeriod } = data;
          ...
          setVocationLeaveNoticePeriod(vactionLeaveNoticePeriod)
        }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const data = await fetchResourceById(session?.user.id);
        if (data) {
          const { vocationLeaves, vocationLeavesAvailable,vactionLeaveNoticePeriod ,vocationLeavesConsumed, sickLeaves, sickLeavesConsumed, sickLeavesAvailable } = data;
          setVocationLeaves(vocationLeaves);
          setVocationLeavesAvailable(vocationLeavesAvailable);
          setVocationLeavesConsumed(vocationLeavesConsumed); // Fixing this to use vocationLeavesConsumed
          setSickLeaves(sickLeaves);
          ...
          console.log({ vocationLeaves, vocationLeavesAvailable, vocationLeavesConsumed, sickLeaves, sickLeavesConsumed, sickLeavesAvailable,vactionLeaveNoticePeriod } )
  ...
  useEffect(() => {
    const fetchHolidays = async () => {
      try {
        const data: HolidaysType[]|undefined = await getAllHolidaysDate(); 
       if (data){
        setHolidays(data); // 
